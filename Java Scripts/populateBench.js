...
//    var cruName = JSON.parse(localStorage.cruName);
    ...
    for (i=0; i < benchMax; i++) {
        for (j=0; j < benchSize; j++) {
            indexStart = benchSize*i;
            benchTMP = "<option value=\"".concat(cruName[indexStart+j].name)+"\">".concat(cruName[indexStart+j].name)+"</option>";
            benchTMPTwo = benchTMPTwo.concat(benchTMP);
        }
        benchList = "<select>"+benchTMPTwo+"</select>";
